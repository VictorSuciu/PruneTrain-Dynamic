...
if args.dataset == 'imagenet':
    ...
    cfg_file = "{}_{}.yaml".format(args.dataset, args.model)
...
with open('configs/path.yaml') as path_config:
   cfg_path = yaml.safe_load(path_config)
if not os.path.exists(cfg_path['model_dir']):
    os.makedirs(cfg_path['model_dir'])

# Reconfigured network file naming and directory to store
arch_name = cfg['base']['arch']+'_'+cfg['base']['description']
arch_out_dir = os.path.join(cfg_path['model_dir'], 'arch', cfg['base']['description'])

skip = []

# Build command line
for cur_epoch in range(0, cfg['base']['epochs'], cfg['pt']['sparse_interval']):
    cmd_line = 'python src/imagenet_pt_fp32.py '
    ...
    cmd_line += ' --checkpoint '            +os.path.join(cfg_path['model_dir'], cfg['base']['description'])
    cmd_line += ' --arch '                  +cfg['base']['arch']
    cmd_line += ' --gpu-id '                +gpu_id
    cmd_line += ' --train_batch '           +str(cfg['base']['train_batch'] *args.num_gpus/4 )
    ...
    cmd_line += ' >> '                      +os.path.join(cfg_path['model_dir'], cfg['base']['description'])+'.log'
    ...
    cfg['base']['resume'] = os.path.join(cfg_path['model_dir'], cfg['base']['description'], 'checkpoint.pth.tar')
