...
def main():
    ...
    #train_dataset = LimitDataset(train_dataset, 10)
    ...
    if args.arch.startswith('alexnet') or args.arch.startswith('vgg'):
        ...
        model = torch.nn.DataParallel(model).cuda().half()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Save the checkpoint (save at every epoch)
...
def _get_group_lasso(model):
    ...
    for name, param in model.named_parameters():
       ...
       if ('weight' in name) and any([i for i in ['conv', 'fc'] if i in name]):
           # ======== Increase by the number of weights per channel
           if param.dim() == 4:
               _in  = param.float().pow(2).sum(dim=[0,2,3])
               _out = param.float().pow(2).sum(dim=[1,2,3])
               ...
               lasso_in_ch.append( param.float().pow(2).sum(dim=[0]) )
...
def train(train_loader, model, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        # Change input to half_precision
        inputs = inputs.half() 
