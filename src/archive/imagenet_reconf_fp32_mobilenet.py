...
parser.add_argument('--lasso_option', default=1, type=int,
                    help='choose additional lasso penalty to non-depthwise convolution') 
parser.add_argument('--reg_ew_conv', default=False, action='store_true',
                    help='Apply regualization to elementwise convolution') 
...
def _get_group_lasso(model):
    ...
    for name, param in model.named_parameters():
        ...
        if ('weight' in name) and any([i for i in ['conv', 'fc'] if i in name]):
            if param.dim() == 4:
                conv_ew = ('mobilenet' in args.arch) and int(name.split('.')[1].split('conv')[1]) %2 == 0

                if ('conv1.' not in name) and not conv_ew:
                    _in = param.pow(2).sum(dim=[0,2,3])
                    lasso_in_ch.append( _in )

                # Apply differnet lasso coefficient to element-wise conv
                if conv_ew and args.reg_ew_conv:
                    _out = param.pow(2).sum(dim=[1,2,3])
                    lasso_conv_dw.append( _out )
                else:
                    _out = param.pow(2).sum(dim=[1,2,3])
    ...
    _lasso_conv_dw       = torch.cat(lasso_conv_dw).cuda()

    if args.lasso_option == 1:
        lasso_penalty_in_ch   = torch.mul(_lasso_in_ch.add(1.0e-8).sqrt().sum(), 3)
        lasso_penalty_out_ch  = torch.mul(_lasso_out_ch.add(1.0e-8).sqrt().sum(), 3)
    else:
        lasso_penalty_in_ch   = torch.mul(_lasso_in_ch.add(1.0e-8).sqrt().sum(), np.sqrt(32))
        lasso_penalty_out_ch  = torch.mul(_lasso_out_ch.add(1.0e-8).sqrt().sum(), np.sqrt(32))
    lasso_penalty_conv_dw = _lasso_conv_dw.add(1.0e-8).sqrt().sum()

    lasso_penalty        = lasso_penalty_in_ch + lasso_penalty_out_ch + lasso_penalty_conv_dw
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        if args.en_group_lasso:
            ...
            if init_batch:
                ...
                with open( os.path.join(coeff_dir, str(args.var_group_lasso_coeff)+'_'+str(args.lasso_option)), 'w' ) as f_coeff:
                    f_coeff.write( str(grp_lasso_coeff.item()) )

            else:
                with open( os.path.join(coeff_dir, str(args.var_group_lasso_coeff)+'_'+str(args.lasso_option)), 'r' ) as f_coeff:
                    for line in f_coeff:
        ...
        if batch_idx % args.print_freq == 0:
            ...
            'Acc@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                epoch+1, batch_idx, len(train_loader), batch_time=batch_time,
