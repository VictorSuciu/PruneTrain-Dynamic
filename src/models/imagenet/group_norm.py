...
r""" Modfied the source code from  https://github.com/chengyangfu/pytorch-groupnormalization
"""
...
class _GroupNormGrp(_BatchNorm):
    def __init__(self, num_features, num_groups=1, eps=1e-5, momentum=0.1,
                 affine=False, track_running_stats=False):
        self.num_groups = int(num_features/num_groups)
        self.track_running_stats = track_running_stats
        super(_GroupNormGrp, self).__init__(int(num_groups), eps,
                                         momentum, affine, track_running_stats)
...
class GroupNorm2d(_GroupNorm):
    r"""
    Maintain the same number of channels per group for different layers
...
class GroupNorm2dGrp(_GroupNormGrp):
    r"""
    Maintain the same number of groups for different layers
    """
